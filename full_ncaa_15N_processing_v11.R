...
hAll_1 <- filter(hAll, Experiment == "NT.19.47.10")
hAll_2 <- filter(hAll, Experiment == "NT.19.47.13")
hAll_3 <- filter(hAll, Experiment == "NT.19.47.16")
...
list_clean_complete <- list_clean_complete %>%
  ...
  select(c("firstID", "Protein.IDs", "meanriBAQ", "sdriBAQ", "Experiment", "corriBAQ", "Met..HPG.site.positions", "riBAQ")) %>% 
  ungroup()
hpg_enriched <- list_clean_complete #create an object for PCA
...
#Make three dataframes, one for each experiment
protE_1 <- filter(protE, Experiment == "NT.20.21.25A")
protE_2 <- filter(protE, Experiment == "NT.20.21.26A")
protE_3 <- filter(protE, Experiment == "NT.20.21.27A")

#Join the six dataframes in all nine possible combinations and calculate enrichment
frodo <- full_join(hAll_1, protE_1, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
sam <- full_join(hAll_1, protE_2, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
merry <- full_join(hAll_1, protE_3, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
pippin <- full_join(hAll_2, protE_1, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
boromir <- full_join(hAll_2, protE_2, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
aragorn <- full_join(hAll_2, protE_3, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
legolas <- full_join(hAll_3, protE_1, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
gimli <- full_join(hAll_3, protE_2, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
gandalf <- full_join(hAll_3, protE_3, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()

#Row bind all nine together, group by firstID and take the mean and sd of enrichment values
tf <- rbind(frodo, sam, merry, pippin, boromir, aragorn, legolas, gimli, gandalf) %>% 
  group_by(firstID) %>% 
  mutate(meanFoldEnri = mean(fold_enrichment, na.rm = T)) %>%
  mutate(sdFoldEnri = sd(fold_enrichment, na.rm = T)) %>%
  select(-c("sdriBAQ_unenriched", "Experiment_unenriched", "riBAQ_unenriched",
             "sdriBAQ_enriched", "Experiment_enriched", "riBAQ_enriched", "fold_enrichment")) %>% 
  ungroup() %>% 
  distinct()

uf <- filter(tf, tagged_unenriched == "Y")
unique(uf$prot_group_HPG_unenriched)
write.csv(tf, "protFoldEnriHPG.csv", row.names = F)
...
#Make three dataframes
aAll_1 <- filter(aAll, Experiment == "NT.19.47.1")
aAll_2 <- filter(aAll, Experiment == "NT.19.47.4")
aAll_3 <- filter(aAll, Experiment == "NT.19.47.7")
...
#Make three dataframes, one for each experiment
protE_1 <- filter(protE, Experiment == "NT.20.92.2A")
protE_2 <- filter(protE, Experiment == "NT.20.21.23A")
protE_3 <- filter(protE, Experiment == "NT.20.21.24A")

#Join the six dataframes in all nine possible combinations and calculate enrichment
frodo <- full_join(aAll_1, protE_1, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
sam <- full_join(aAll_1, protE_2, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
merry <- full_join(aAll_1, protE_3, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
pippin <- full_join(aAll_2, protE_1, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
boromir <- full_join(aAll_2, protE_2, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
aragorn <- full_join(aAll_2, protE_3, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
legolas <- full_join(aAll_3, protE_1, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
gimli <- full_join(aAll_3, protE_2, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
gandalf <- full_join(aAll_3, protE_3, by = "firstID", suffix = c("_unenriched", "_enriched")) %>% 
  group_by(firstID) %>% 
  mutate(fold_enrichment = (riBAQ_enriched - riBAQ_unenriched)/(riBAQ_enriched + riBAQ_unenriched)) %>% 
  ungroup()
...
write.csv(tf, "protFoldEnriAHA.csv", row.names = F)
...
#Make three dataframes
protE_1 <- filter(protE, Experiment == "NT.20.21.25B")
protE_2 <- filter(protE, Experiment == "NT.20.21.26B")
protE_3 <- filter(protE, Experiment == "NT.20.21.27B")
...
tf <- rbind(frodo, sam, merry, pippin, boromir, aragorn, legolas, gimli, gandalf) %>% 
  ...
  ungroup() %>% 
  distinct()
write.csv(tf, "protFoldEnriHPGCleaved.csv", row.names = F)
...
#Make three dataframes, one for each experiment
protE_1 <- filter(protE, Experiment == "NT.20.21.22B.1")
protE_2 <- filter(protE, Experiment == "NT.20.21.23B")
protE_3 <- filter(protE, Experiment == "NT.20.21.24B")
...
tf <- rbind(frodo, sam, merry, pippin, boromir, aragorn, legolas, gimli, gandalf) %>% 
  ...
  ungroup() %>% 
  distinct()
write.csv(tf, "protFoldEnriAHACleaved.csv", row.names = F)
...
#Make three dataframes
protE_1 <- filter(protE, Experiment == "NT.20.21.25C")
protE_2 <- filter(protE, Experiment == "NT.20.21.26C")
protE_3 <- filter(protE, Experiment == "NT.20.21.27C")
...
tf <- rbind(frodo, sam, merry, pippin, boromir, aragorn, legolas, gimli, gandalf) %>% 
  ...
  ungroup() %>% 
  distinct()
write.csv(tf, "protFoldEnriHPGCleavedPostDige.csv", row.names = F)
...
#Make three dataframes, one for each experiment
protE_1 <- filter(protE, Experiment == "NT.20.21.22C")
protE_2 <- filter(protE, Experiment == "NT.20.21.23C")
protE_3 <- filter(protE, Experiment == "NT.20.21.24C")
...
tf <- rbind(frodo, sam, merry, pippin, boromir, aragorn, legolas, gimli, gandalf) %>% 
  ...
  ungroup() %>% 
  distinct()
write.csv(tf, "protFoldEnriAHACleavedPostDige.csv", row.names = F)

######investigating enriched/unenriched data set#####
aEnri <- read.csv("protFoldEnriAHA.csv", stringsAsFactors = F)
aClea <- read.csv("protFoldEnriAHACleaved.csv", stringsAsFactors = F)
aCleaP <- read.csv("protFoldEnriAHACleavedPostDige.csv", stringsAsFactors = F)
hEnri <- read.csv("protFoldEnriHPG.csv", stringsAsFactors = F)
hClea <- read.csv("protFoldEnriHPGCleaved.csv", stringsAsFactors = F)
hCleaP <- read.csv("protFoldEnriHPGCleavedPostDige.csv", stringsAsFactors = F)
#digested on resin
...
m <- filter(aEnri, meanFoldEnri > 0) %>% 
  filter(tagged_unenriched == "Y")
m <- length(unique(m$firstID))
n <- filter(hEnri, meanFoldEnri > 0) %>% 
  filter(tagged_unenriched == "Y")
...
m <- filter(aClea, meanFoldEnri > 0) %>% 
  filter(tagged_unenriched == "Y")
m <- length(unique(m$firstID))
n <- filter(hClea, meanFoldEnri > 0) %>% 
  filter(tagged_unenriched == "Y")
n <- length(unique(n$firstID))
...
#of those with enrichment > 0, how many are tagged in the unenriched set?
m <- filter(aCleaP, meanFoldEnri > 0) %>% 
  filter(tagged_unenriched == "Y")
m <- length(unique(m$firstID))
n <- filter(hCleaP, meanFoldEnri > 0) %>% 
  filter(tagged_unenriched == "Y")
n <- length(unique(n$firstID))
...
hEnri2 <- hEnri %>% 
  select(c("firstID", "meanFoldEnri", "sdFoldEnri", "tagged_unenriched", "tagged_enriched",
           "meanriBAQ_unenriched", "meanriBAQ_enriched", "prot_group_HPG_unenriched", "prot_group_HPG_enriched")) %>% 
  filter(is.na(meanriBAQ_enriched) == F | tagged_unenriched == "Y")
aEnri2 <- aEnri %>% 
  select(c("firstID", "meanFoldEnri", "sdFoldEnri", "tagged_unenriched", "tagged_enriched",
           "meanriBAQ_unenriched", "meanriBAQ_enriched", "prot_group_AHA_unenriched", "prot_group_AHA_enriched")) %>% 
  filter(is.na(meanriBAQ_enriched) == F | tagged_unenriched == "Y")
...
omega <- cmplt13 %>% 
  select(c("prot", "LPF", "LPF.riBAQ", "meanFoldEnri_AHA", "meanFoldEnri_HPG", "tagged_unenriched_AHA",
...
bilbo <- cmplt13 %>% 
  select(c("prot", "n_pep_15N", "LPF", "LPF.riBAQ", "n_tagged_pep_HPG", "meanFoldEnri_HPG", "meanriBAQ_enriched_HPG",
           "meanFoldEnri_AHA", "meanriBAQ_enriched_AHA")) %>% 
  distinct()
